...
export default function QuotePage() {
  ...
  function buildPreviewText() {
    ...
    const footer = [
      "",
      (draft?.serviceTotal != null ? `Servicios (instalación + extras): ${fmtMoney(draft.serviceTotal)} (${draft.numVehicles ?? 1} vehículo(s))` : null),
      (draft?.grandTotal != null ? `Total estimado (llantas + servicios): ${fmtMoney(draft.grandTotal)}` : null),
      msgOutro
    ].filter(Boolean);
