...
const defaults = await getSettingsDefaults();
    ...
    const numVehicles = Math.max(1, Array.isArray(vehicles) ? vehicles.length : 1);
const serviceTotal = (Number(install ?? 0) + Number(extras ?? 0)) * numVehicles;

// Estimated tires total (cheapest option per line)
...
// Include vehicle+size blocks
const bodyText = (perLineResults ?? []).map((lr: any) => {
  const make = (lr as any).vehicleMake ?? (lr as any).vehicle_make ?? "";
  const model = (lr as any).vehicleModel ?? (lr as any).vehicle_model ?? "";
  const year = (lr as any).vehicleYear ?? (lr as any).vehicle_year ?? "";
  const vehicleTxt = [make, model, year].filter(Boolean).join(" ");

  const opts = (lr.options ?? []);
  if (!opts.length) {
    return `• ${vehicleTxt ? `Vehículo: ${vehicleTxt}\n` : ""}Medida: ${lr.size} (x${lr.requestedQty})\n  Sin opciones con stock suficiente`;
  }

  const lines = opts.map((o: any, i: number) => {
    const qtyText = o.limited ? ` (Disponible x${o.quotedQty})` : "";
    return `  #${i + 1} [${o.tier}] ${o.brand} ${o.model ?? ""} ${o.loadSpeed ?? ""} | $${o.priceEach} c/u | Total: $${o.totalTires}${qtyText}`;
  }).join("\n");
...
const servicesText =
`\n\nServicios (instalación + extras): $${serviceTotal.toFixed(2)} (${numVehicles} vehículo(s))
Total estimado (llantas + servicios): $${grandTotal.toFixed(2)}`;

const whatsappText =
  header + "\n" + bodyText + servicesText + "\n\nResponde con el número de opción (por medida) para apartarla.";
